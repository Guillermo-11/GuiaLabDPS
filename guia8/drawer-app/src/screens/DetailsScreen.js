...
export default function DetailsScreen({ navigation }) {
  ...
  const [projects, setProjects] = useState([
    ...
    { id: '3', name: 'Proyecto 4', manager: 'Guillermo Granadino', contact: 'guille@example.com', socialMedia: ['x', 'instagram'] },
  ]);
    ...
    <View style={styles.projectItem}>
      ...
      <View style={styles.socialMediaIcons}>
        {item.socialMedia.map((social, index) => (
          <Image key={index} source={getSocialMediaIcon(social)} styles={styles.socialMediaIcon} />
        ))}
      </View>
    </View>
  );
